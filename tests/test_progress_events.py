...
def _make_manager(tmp_path: Path, *, progress: bool = False, scrape: bool = False) -> CSVLogsManager:
    ...
    files = CSVFiles(
        unsupported_urls=tmp_path / "unsupported.csv",
        download_errors=tmp_path / "download_errors.csv",
        scrape_errors=tmp_path / "scrape_errors.csv",
        last_forum_post=tmp_path / "last.csv",
        jsonl_file=main_log.with_suffix(".results.jsonl"),
...
def test_csv_files_iter_skips_none(tmp_path: Path) -> None:
    ...
    files = CSVFiles(
        unsupported_urls=tmp_path / "b.csv",
        download_errors=tmp_path / "c.csv",
        scrape_errors=tmp_path / "d.csv",
        last_forum_post=tmp_path / "a.csv",
        jsonl_file=main.with_suffix(".results.jsonl"),
...
async def test_progress_event_creates_parent_dir(tmp_path: Path) -> None:
    ...
    files = CSVFiles(
        unsupported_urls=tmp_path / "a.csv",
        download_errors=tmp_path / "c.csv",
        scrape_errors=tmp_path / "d.csv",
        last_forum_post=tmp_path / "b.csv",
        jsonl_file=main.with_suffix(".results.jsonl"),
